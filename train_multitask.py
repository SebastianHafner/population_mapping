...
def run_training(cfg):
    ...
    net = networks.PopulationNetMultiTask(cfg.MODEL)
    net.to(device)
    optimizer = optim.AdamW(net.parameters(), lr=cfg.TRAINER.LR, weight_decay=0.01)

    criterion_pop = loss_functions.get_criterion(cfg.MODEL.LOSS_TYPE)
    criterion_hab = torch.nn.CrossEntropyLoss()
    ...
    for epoch in range(1, epochs + 1):
        ...
        loss_set, loss_pop_set, loss_inhab_set, pop_set, inhab_set = [], [], [], [], []
        ...
        for i, batch in enumerate(dataloader):
            ...
            x = batch['x'].to(device)
            y_pop = batch['y'].to(device)
            y_inhab = (y_pop == 0).squeeze()

            y_hat_pop, logits_inhab = net(x)

            loss_pop = criterion_pop(y_hat_pop, y_pop.float())
            loss_inhab = criterion_hab(logits_inhab, y_inhab.long())

            loss = loss_pop + loss_inhab

            loss.backward()
            optimizer.step()

            loss_set.append(loss.item())
            loss_pop_set.append(loss_pop.item())
            loss_inhab_set.append(loss_inhab.item())
            pop_set.append(y_pop.flatten())
            inhab_set.append(y_inhab.flatten())
            ...
            if global_step % cfg.LOG_FREQ == 0 and not cfg.DEBUG:
                ...
                wandb.log({
                    ...
                    'inhab_percentage': inhab_percentage,
                    'time': time,
                    'step': global_step,
                    'epoch': epoch_float,
                })
